转载：https://cloud.tencent.com/developer/article/1817817
...
- 访问程序集中类型的元数据： 其中包括像完整类型名和成员名这样的构造，以及对一个构造进行修饰的任何特性。·使用元数据在运行时动态调用类型的成员，而不是使用编译时绑定。
- .NET通过 C# 语言提供的诸多服务（例如动态绑定、序列化、数据绑定和 Remoting）都是依托于元数据的： 我们的应用程序可以充分地利用这些元数据，甚至可以通过自定义特性向元数据中添加信息。我们甚至可以通过 System.Reflection.Emit 命名空间中的类在运行时动态创建新的元数据和可执行IL（中间语言）指令。
...
```
Assembly ass = Assembly.LoadFrom(@"C:\ProgramFiles\dotnet\packs\Microsoft.NETCore.App.Ref\3.0.0\ref\netcoreapp3.0\System.Console.dll");
...
关于这方面的知识，可以参考笔者的另一篇文章：[https://cloud.tencent.com/developer/article/1817750](https://cloud.tencent.com/developer/article/1817750?from_column=20421&from=20421)
...
`Dictionary`2 ，这个 `后表示此泛型类型有多少个泛型参数。
